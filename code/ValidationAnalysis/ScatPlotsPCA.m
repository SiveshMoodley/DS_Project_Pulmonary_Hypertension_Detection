function [AUCROCPRC,YIcsr,Rcsr,PVDt,newSRCC] = ScatPlotsPCA(numModes,mPAP,scores,mPAPcutoff,PercentVar,PHclass)
...
for i = 1:H
    ...
    if mPAPcutoff == 25
        ...
        xlim([0 1])
        ylim([0 1])
        legend("","Random Classifier Line","Youden's Index (J Statistic)",'interpreter','latex')
    ...
    if mPAPcutoff == 25
        ...
        xlim([0 1])
        ylim([0 1])
        legend("","Baseline Classifier Line","Maximum F$B$ Score",'interpreter','latex')
        set(gca,'TickLabelInterpreter','latex')
        set(gcf,'Position',[600 400 800 400])
