function [AUCROCPRC,YIcsr,Rcsr,PVDt,newSRCC,csr] = ScatPlotsPCA(numModes,mPAP,scores,mPAPcutoff,PercentVar,PHclass)
...
SRCC = array2table(SRCC, 'VariableNames', ["PCA Shape Mode No.","Spearman's Rank Correlation Coefficient","Spearman's P-Value"]);
...
if H ==0
    msg = "No significant PCA shape modes were found to correlate to Pulmonary Hypertension.";
    error(msg)
end
...
for i = 1:H
    ...
    % Random and Base Classifier Line
    ...
    if mPAPcutoff == 25
        ...
        title(strjoin(["PCA Shape Mode ",num2str(SM)," Receiver Operating Characteristic Curve"]),'interpreter','latex')
        ...
        xlim([10 70])
        title(strjoin(["Quantity of PCA Shape Mode ",num2str(SM)," in each Patient and their associated Mean Pulmonary Artery Pressure"]),'interpreter','latex')
        subtitle("Pulmonary Hypertension Classification, mPAP = 25mmHg",'interpreter','latex')
        xlabel("mPAP (mmHg)",'interpreter','latex')
        ylabel("PCA Shape Mode Quantity",'interpreter','latex')
    ...
    %% PRC Shape Mode Amount Cutoff and find new Actual and Successfull Classification Rates
    ...
    if mPAPcutoff == 25
        ...
        title(strjoin(["PCA Shape Mode ",num2str(SM)," Precision Recall Curve"]),'interpreter','latex')
        subtitle("Pulmonary Hypertension Classification, mPAP = 25mmHg",'interpreter','latex')
...
PVDt = table(SMn,PVD);
PVDt.Properties.VariableNames = ["PCA Shape Mode No.","Percentage Variance of the Dataset (%)"];

% Create table of AUCROC and AUC-ARC data and their representative shape mode numbers
AUCROCPRC = table(SMn,AUC,PRC);
AUCROCPRC.Properties.VariableNames = ["PCA Shape Mode No.","AUCROC Value","AUC-PRC Value"];
...
YIcsr.Properties.VariableNames = ["PCA Shape Mode No."," Actual Successful Classification Rate (%)"," Theoretical Successful Classification Rate (%)"];

% Create table of refined classification success rates and their representative shape mode numbers
Rcsr = table(SMn,scrA,scrT);
Rcsr.Properties.VariableNames = ["PCA Shape Mode No."," Actual Successful Classification Rate (%)"," Theoretical Successful Classification Rate (%)"];

% Create table of refined classification success rates and their representative shape mode numbers
csr = table(SMn,sucRaA,sucRaT,SensD,SpecD);
csr.Properties.VariableNames = ["PCA Shape Mode No."," Actual Successful Classification Rate (%)"," Theoretical Successful Classification Rate (%)","Sensitivity","Specificity"];
